...
// full duration.
void expect_no_additional_sends(FakeNetwork& net, std::size_t expected_count,
...
Price drain_to(PriceUpdateService& svc, FakeNetwork& net, StationId station, Price target,
...
TEST_F(PriceUpdateServiceTest, CoalescesRapidUpdatesWhileInFlight) {
    ...
    for (const auto& m : msgs) {
        EXPECT_NE(m.price, 120) << "an intermediate price should never be put on the wire";
    }
...
TEST_F(PriceUpdateServiceTest, EventuallyConvergesToLatestPriceDespiteManyChanges) {
...
TEST_F(PriceUpdateServiceTest, StaleOrDuplicateAcknowledgementIsIgnoredSafely) {
    svc.set_price(1, 100);
    ASSERT_TRUE(net.wait_for_count(1));
    const auto first = net.snapshot()[0];

    svc.set_price(1, 200); // superseding value, still queued behind the in-flight send
    expect_no_additional_sends(net, 1);
...
TEST_F(PriceUpdateServiceTest, RaceBetweenSetPriceAndAcknowledgeNeverLosesUpdate) {
    svc.set_price(1, 1);
    ASSERT_TRUE(net.wait_for_count(1));
...
TEST_F(PriceUpdateServiceTest, ConcurrentMultiStationStressConverges) {
    ...
    for (StationId s : station_ids) {
        writers.emplace_back([&svc = svc, s] {
            for (Price p = 1; p <= kFinal; ++p) {
                svc.set_price(s, p);
            }
        });
    ...
    EXPECT_TRUE(converged) << "not every station's latest sent price reached " << kFinal;
}
